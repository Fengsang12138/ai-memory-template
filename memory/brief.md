# Memory Brief — 快速上下文（低 token）

last_updated: 2026-04-29T09:12:13Z

> 默认加载本文件；跨 agent 接手时再读 `memory/handoff.md` + `memory/activeContext.md` + `memory/tasks.md`。
> 需要历史细节时，优先 `rg` 在 `memory/decisions.md` / `memory/progress.md` / `memory/archive/` 内检索，避免整文件全读。

## 当前焦点（TL;DR）
- （填写当前项目最重要的目标）

## 当前交接（Handoff）
- （无）

## 常用入口
- （列出本项目的常用入口，例如启动命令、构建命令、主文档、关键脚本）

## 关键范围（Scope）
- （列出本轮需要关注的目录、模块、服务或文档）

## 近期待办（Top 5）
- [ ] (id: t00000000000000, p: med, status: todo, owner: shared) 示例任务：初始化项目目标、入口命令与首批待办

## 最近进展（Last 3）
- （无）

## 近期关键决策（Last 3）
- 2026-03-10T00:00:00Z：模板初始化完成，可开始记录项目级关键决策。

## 维护约定（省 token）
- 本文件只保留短摘要；即时交接入口是 `memory/handoff.md`。
- 长历史入口在 `memory/progress.md` / `memory/decisions.md` / `memory/archive/`。
- 每次任务结束：正式产出写 `progress.md`；重要取舍写 `decisions.md`；然后刷新 `brief.md`。
- 原始或 session-only 会话记录默认留在本地或 `memory/.session-cache.md`，不要自动塞进正式进展。
- 脚本：`scripts/memory/handoff`（更新交接）、`scripts/memory/brief-refresh`（刷新本文件）、`scripts/memory/prune`（归档旧记录）。
