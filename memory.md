...
2. When resuming unfinished work, switching agents, or seeing dirty files, read [`memory/handoff.md`](./memory/handoff.md).
3. Then read [`memory/activeContext.md`](./memory/activeContext.md) and [`memory/tasks.md`](./memory/tasks.md) if you need active detail.
4. Only read [`memory/decisions.md`](./memory/decisions.md), [`memory/progress.md`](./memory/progress.md), or [`memory/glossary.md`](./memory/glossary.md) when you need deeper history or terminology.
...
- Treat Codex runtime memory, and optional Claude runtime memory when enabled, as separate from this repository memory bank unless a workflow explicitly syncs a summary back.

## Write-Back Reminder

- Update [`memory/progress.md`](./memory/progress.md) after meaningful work.
- Update [`memory/handoff.md`](./memory/handoff.md) when another collaborator or agent needs to continue from the current state.
